# Cloudflare 切り替え手順書（実施済み・記録）

> **この手順は 2026-08-30 に実施済みで、AWS 側の資源は撤去されている。**
> 参照するコマンドの多く（ECS のタスク定義、`cutover-migrate.json`、`deploy-bluegreen.sh`）は
> もう存在しない。切り戻し先も無い。当時どう進めたか、何につまずいたかの記録として残している。
> 付録の一覧は Workers 上で同種の問題を疑うときに今も使える。

AWS（ALB + ECS Fargate + RDS）から Cloudflare Workers + Supabase へ本番を切り替えた手順。
実測で所要は約50分、利用者から見えるダウンタイムは十数分だった。
