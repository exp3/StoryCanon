  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-semibold">伏線一覧</h1>
        <Link className="text-sm text-[#4b4b45] underline" href={`/projects/${projectId}`}>
          作品詳細へ戻る
        </Link>
      </div>
      <p className="mb-4 text-sm leading-6 text-[#4b4b45]">
        伏線はChatGPT連携から保存されます。ここでは一覧の確認のみ行えます。
      </p>
      {foreshadowings.length === 0 ? (
        <p className="text-sm text-[#555]">まだ伏線がありません。</p>
      ) : (
            ...
            <li key={item.id} className="rounded border bg-white p-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="shrink-0 text-xs text-[#666]">
                  {item.status} ・重要度 {item.importance}
                </span>
              </div>
              <p className="mt-2 text-sm leading-6 text-[#555]">{item.description}</p>
              {item.plannedResolution ? (
                <p className="mt-2 text-sm leading-6 text-[#555]">回収予定: {item.plannedResolution}</p>
              ) : null}
            </li>
          ))}
        </ul>
